...
- **Ando.CopyArtifactsToHost()**: Copy build outputs from container to host

## Artifact Operations

When running builds in Docker containers, build outputs exist inside the container.
The Ando API allows copying these files back to the host system.
...
// Copy specific files
Ando.CopyArtifactsToHost("bin/Release/app", "./output/app");
```
...
3. `Ando.CopyArtifactsToHost()` registers files to copy
